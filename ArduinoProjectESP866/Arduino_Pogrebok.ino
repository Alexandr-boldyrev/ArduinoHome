// Финальная версия программы погребок
...
struct meteo_home
{
  ...
  float temp_cellar = 5;  //Темпреатура погреб
...
char server_php_raspberry[] = "192.168.1.45"; // PHP server
char server_php_debian[] = "37.193.0.199"; // PHP server
...
const int interval_send_php = 300000; // 5 минут
const int interval_lcd = 5000; // 5 сек
...
void relay_control() {
  ...
  if (millis() - get_send_php > interval_send_php  || get_send_php > millis()) { // отправляем данн на сервер каждые 10 минут... даже если обнулиться millis
    sendData(meteo.temp_street_max, meteo.temp_street_min, meteo.pressure, meteo.temp_cellar, meteo.temp_street, meteo.temp_home, meteo.kwt_full, meteo.time_power, meteo.count_tarn);
    get_send_php = millis();
    //Serial.println("SEND DATA SERVER: PHP");
  }
}
////////////////////////////////////////////////////////////////////////////////////////////
//Отправляем данные на сервер Raspberry PI
////////////////////////////////////////////////////////////////////////////////////////////
void sendData(float temp_street_max, float temp_street_min, float pressure, float temp_cellar, float temp_street, float temp_home, float kwt_full, float time_power, int count_tarn) {
  for (int i = 0; i < 2 ; i++) {
    if (i == 0)
      client.connect(server_php_raspberry, 80);
    else
...
void handleRoot() {
  ...
  <body>\
   <h2><font color=\"black\">Программа контроля температуры в погребе Arduino ESP8266 Web Server<br />";
  ...
  if (digitalRead(SSR40DA_pin)) string_html += "Подогрев: <i><font color=\"red\">Вкл.</font></i><br />";
  else string_html += "Подогрев: <i><font color=\"green\">Выкл.</font></i><br />";
...
float get_temp(int n) {
  ...
  if (n == 2) //Датчик в погребе индекс для переопределения датчика поменять на 2
